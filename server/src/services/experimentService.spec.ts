...
describe('ExperimentService Tests',() =>{
    ...
    describe('add test plan' , () => {
        ...
        it('success- add one testPlan', async () => {
           const res = await experimentService.addTestPlan(testPlanName,true, [{experimentName: expName, formId:formName }]);
           expect(res.status).toEqual(0);
        });

        it('fail- experiment name not exist', async () => {
            const {status, error} = await experimentService.addTestPlan(testPlanName, true, [{experimentName: 'badExpName', formId: formName }]);
            expect(status).toEqual(ERROR_STATUS.OBJECT_NOT_EXISTS);
            expect(error).toEqual(ERRORS.EXP_NOT_EXISTS);
        });

        it('fail- form name does not exist', async () => {
            const {status, error} = await experimentService.addTestPlan(testPlanName, true, [{experimentName: expName, formId:'badFormName' }]);
            expect(status).toEqual(ERROR_STATUS.OBJECT_NOT_EXISTS);
            expect(error).toEqual(ERRORS.FORM_NOT_EXISTS);
        });

        it('fail- testPlan name already exist', async () => {
            await experimentService.addTestPlan(testPlanName, true, [{experimentName: expName, formId: formName }]);
            const {status, error} = await experimentService.addTestPlan(testPlanName, true, [{experimentName: expName, formId:formName }]);
            expect(status).toEqual(ERROR_STATUS.NAME_NOT_VALID);
